...
FormatTag(client, String:buffer[], char[] temp, maxlength)
{
	...
	if(0 < client <= MaxClients)
	{
		...
		ReplaceString(temp, maxlength, "{name}", sName, true);
		ReplaceString(buffer, maxlength, sName, temp, true);
	}

	ReplaceString(buffer, maxlength, "{start}", " \x01\x0B", true);
}
...
GetChatName(client, String:buffer[], maxlength)
{
	if((g_ClientUseCustom[client] & CC_HASCC) && (g_ClientUseCustom[client] & CC_NAME) && GetConVarBool(g_hUseCustomChat))
	{
		...
		if(idx != -1)
		{
			char temp[MAXLENGTH_NAME];
			GetArrayString(g_hCustomNames, idx, temp, sizeof(temp));
			FormatTag(client, buffer, temp, maxlength);
		}
	}
	else if(GetConVarBool(g_hUseChatRanks))
	{
		...
		for(new i=0; i<iSize; i++)
		{
			...
			{
				char temp[MAXLENGTH_NAME];
				GetArrayString(g_hChatRanksNames, i, temp, sizeof(temp));
				FormatTag(client, buffer, temp, maxlength);
				return;
			}
		}
...
EnableCustomChat(const String:sAuth[])
{
	...
	FormatEx(query, sizeof(query), "UPDATE players SET ccuse=%d, ccname='{name}', ccmsgcol='{white}' WHERE SteamID='%s'",
		CC_HASCC|CC_MSGCOL|CC_NAME,
		sAuth);
	SQL_TQuery(g_DB, EnableCC_Callback, query);

	PushArrayString(g_hCustomSteams, sAuth);
	PushArrayString(g_hCustomNames, "{name}");
	PushArrayString(g_hCustomMessages, "{white}");
...
public Action:SM_DisableCC(client, args)
{
	...
	{
		PrintColorText(client, "%s%ssm_disablecc example: \"sm_disablecc STEAM_1:1:12345\"",
			g_msg_start,
			g_msg_textcol);
	}
...
public Menu_CCList(Handle:menu, MenuAction:action, param1, param2)
{
	if (action == MenuAction_Select)
	{
		...
		PrintColorText(param1, " %sCCMessage: %s%s", g_msg_textcol, g_msg_varcol, expInfo[3]);
...
LoadChatRanks()
{
	...
	if(!FileExists(sPath))
	{
		...
		WriteFileLine(hFile, "\"0-0\"     \"{white}[{team}Unranked{white}] {name}\"");
		WriteFileLine(hFile, "\"1-1\"     \"{white}[{green}Master{white}] {name}\"");
		WriteFileLine(hFile, "\"2-2\"     \"{white}[{red}Champion{white}] {blue}{name}\"");
		CloseHandle(hFile);
	}
